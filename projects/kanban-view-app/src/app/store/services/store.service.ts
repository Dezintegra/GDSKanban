...
export class TasksStoreService {
  ...
  private taskNumberFilterMask: string;
  ...
  public setTaskNumberFilter(taskNumberFilterMask: string) {
    this.taskNumberFilterMask = taskNumberFilterMask;
  ...
  private pushFilteredDate(): void {
    ...
    this.allTasks.forEach(task => {
      ...
      if (!this.responsibleFilterMask && !this.taskNumberFilterMask) resultSet.push(task);

      if (!this.taskNumberFilterMask 
        || task.id.indexOf(this.taskNumberFilterMask) > -1) {
  ...
  public getData() {
    gapi.client.sheets.spreadsheets.values.get({
      ...
      range: 'Замечания / вопросы!A3:AC3000',
    }).then((response) => {
  ...
  public getFields() {
    gapi.client.sheets.spreadsheets.values.get({
      spreadsheetId: '1AlihhnzwKhJLWxeCYY3eTwBfozpUGkS_VeDJk6AZrco',
      range: 'Замечания / вопросы!A1:AC1',
    }).then((fields) => {
